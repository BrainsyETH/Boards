...
export default function RiverHeader({ river, condition }: RiverHeaderProps) {
  ...
  const getNavigabilityStatus = () => {
    if (!condition) return { label: 'Unknown', color: 'bg-bluff-400', icon: '?' };
    
    switch (condition.code) {
      case 'optimal':
        return { label: 'Navigable', color: 'bg-emerald-500', icon: '✓' };
      case 'low':
      case 'very_low':
      case 'too_low':
      ...
      default:
        return { label: 'Unknown', color: 'bg-bluff-400', icon: '?' };
    }
  };
  ...
  return (
    <div className="bg-gradient-to-br from-ozark-900 via-ozark-800 to-ozark-900 text-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <button
          onClick={() => router.push('/')}
          className="mb-4 text-river-300 hover:text-white transition-colors flex items-center gap-2"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          ...
          <div className="flex-1">
            ...
            {river.description && (
              <p className="text-river-200 text-lg mb-4">{river.description}</p>
            )}
            
            {/* River Stats */}
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center gap-2">
                <span className="text-river-300">Length:</span>
                <span className="font-semibold">{river.lengthMiles.toFixed(1)} miles</span>
              </div>
              {river.region && (
                <div className="flex items-center gap-2">
                  <span className="text-river-300">Region:</span>
                  <span className="font-semibold">{river.region}</span>
                </div>
              )}
              {river.difficultyRating && (
                <div className="flex items-center gap-2">
                  <span className="text-river-300">Difficulty:</span>
          ...
          <div className="flex flex-col items-end gap-4">
            ...
            {/* Gauge Summary */}
            {condition && (
              <div className="bg-ozark-700/50 backdrop-blur-sm rounded-xl px-4 py-3 min-w-[200px]">
                <p className="text-xs text-river-300 mb-1">USGS Gauge</p>
                {condition.gaugeName && (
